...
ENV PATH="/opt/spark/bin:/opt/hadoop/bin:/opt/hive/bin:${PATH}"
...
RUN \
  echo "Download and install Hive" && \
...
RUN \
  ...
  echo "Keep this until this lands: https://issues.apache.org/jira/browse/HIVE-22915" && \
    ...
    cp /opt/hadoop-$HADOOP_VERSION/share/hadoop/hdfs/lib/guava-27.0-jre.jar /opt/apache-hive-$HIVE_VERSION-bin/lib/
RUN \
